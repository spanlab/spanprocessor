...
class Reconstruct_Anat_Lucas(AfniProcess):
    ...
    def commands(self):
        ...
        isvalues = self._isparse_lucas(axiinfo)
        os.chdir(currentdir)
        return {'isvalues':isvalues,'dset_out':anat_name}
...
class Reconstruct_Funcs_CNI(AfniProcess):
    ...
    def commands(self):
        ...
        return {'dset_out':[x for x in self.dset_labels],'suffix_out':self.suffix_out}
...
class Reconstruct_Funcs_Lucas(AfniProcess):
    ...
    def _lucas_3dtcat(self,pfiles_trs,spiral=True,suffix_out='_crop'):
    ...
    def _lucas_3dtshift(self,suffix_out='_cropts',suffix_in='_crop'):
    ...
    def command(self):
        ...
        if not len(self.dset_labels) == len(self.pfiles_by_func):
            ...
            suffix1 = self._lucas_3dtcat(pfiles_trs)
            suffix2 = self._lucas_3dtshift(dset_labels,suffix_in=suffix1)
        return {'dset_out':[(x+suffix2) for x in self.dset_labels],'suffix_out':suffix2}
